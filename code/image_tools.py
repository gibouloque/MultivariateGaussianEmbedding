...
def edge_crop(Z, threshold, cropH, cropW,grid, n_crop=1, no_edge=False):
    ...
    z_app = filters.convolve(Z, phi)
    z_det = filters.convolve(Z, psi)
    ...
    if not no_edge:
        if candidates_score.shape[0]==0:
            ...
            best_idx = np.argpartition(candidates_score, -n_crop)[-n_crop:]#np.argmax(candidates_score)
            Zs = np.zeros((n_crop, cropH,cropW))
            for i in np.arange(n_crop):
                Zs[i,:,:] = Z[candidates[best_idx][i,:][0]:candidates[best_idx][i,:][0]+cropH, candidates[best_idx][i,:][1]:candidates[best_idx][i,:][1]+cropW]
                
            return(Zs ,candidates[best_idx])
    else:
        if candidates_score.shape[0]==0:
            Z2 = center_crop(Z, cropH, cropW)
            return(Z2)
