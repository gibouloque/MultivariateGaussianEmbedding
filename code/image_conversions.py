...
def rolling_row_scan(M,k=3, block_shift=(0,0), add_margin=False, flatten_blocks=True, stride=2, clipped=True):
    """
    View of an image as macro blocks of k*8 X k*8 elements with a row scan
    k : number of block in each column/row
    block_shift : how many block to skip at the beggiing of the image, use to sample lattices 
    ...
    if add_margin:
        ...
        if clipped:
            ...
            M_roll_view = view_as_windows(M, (8*k + 2, 8*k+ 2), step=8*stride)
        ...
        if clipped:
            ...
            M_roll_view = view_as_windows(M, (8*k,8*k), step=8*stride)
    if flatten_blocks:
        if k == 1:
